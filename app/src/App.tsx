...
function App() {
  ...
  useEffect(() => {
    ...
    if (pendingHireTasksRef.current.length === 0 || currentCount <= prevCount)
      return;
  ...
  const handleApplyProposals = useCallback(
    ...
    [
      gameState.staff,
      gameState.inventoryState.items,
      fireStaff,
      setProductPrice,
      startMarketingActivity,
      stopMarketingActivity,
  ...
  const setupSteps = [
    {
      ...
      label: "选择品牌",
      desc: "加盟或自主",
      completed: !!gameState.selectedBrand,
      icon: Store,
    },
    {
      id: "location",
      label: "选址决策",
      desc: "区位与地段",
      completed: !!gameState.selectedLocation && !!gameState.selectedAddress,
      icon: MapPin,
    },
    {
      id: "decoration",
      label: "装修风格",
      desc: "风格与档次",
      completed: !!gameState.selectedDecoration,
      icon: Paintbrush,
    },
    {
      id: "product",
      label: "选品策略",
      desc: "最多选5种",
      completed: gameState.selectedProducts.length > 0,
      icon: Utensils,
    },
    {
      id: "staff",
      label: "人员配置",
      desc: "招聘员工",
      completed: gameState.staff.length > 0,
      icon: Users,
    },
  ];
                  ...
                  <div className="flex gap-2 mb-4">
                    {setupSteps.map((step) => {
                      ...
                      `}
                        ...
                        >
                          <Icon
                            className={`w-5 h-5 ${isActive ? "text-orange-500" : step.completed ? "text-emerald-500" : "text-slate-500"}`}
                          />
                          <span
                            className={`text-xs ${isActive ? "text-orange-400" : step.completed ? "text-emerald-400" : "text-slate-500"}`}
                          >
                            {step.label}
                          </span>
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <p className="text-sm text-slate-400">
                        ...
                        <span className="text-orange-400 font-mono">
                          ¥{(gameState.totalInvestment / 10000).toFixed(1)}万
                        </span>
                        ，剩余现金:{" "}
                        <span className="text-emerald-400 font-mono">
                          ¥{(gameState.cash / 10000).toFixed(1)}万
                        </span>
                      </p>
                      <button
                ...
                <div className="mt-6">
                  {activeSetupStep === "brand" && (
                    <BrandSelection
                      ...
                      cash={
                        gameState.cash +
                        (gameState.selectedBrand?.franchiseFee || 0)
                      }
                    />
                  )}
                  ...
                  {activeSetupStep === "product" && (
                    <ProductSelection
                      ...
                      supplyCostModifier={
                        gameState.selectedBrand?.supplyCostModifier
                      }
                      allowedCategories={
                        gameState.selectedBrand?.allowedCategories
                      }
                    />
                  )}
                ...
                >
                      ...
                      [
                        {
                          ...
                          label: "库存",
                          icon: Package,
                          level: PANEL_UNLOCK_CONFIG.inventory,
                        },
                        {
                          id: "marketing" as const,
                          label: "营销",
                          icon: Megaphone,
                          level: PANEL_UNLOCK_CONFIG.marketing,
                        },
                        {
                          id: "finance" as const,
                          label: "财务",
                          icon: Calculator,
                          level: PANEL_UNLOCK_CONFIG.finance,
                        },
                        {
                          id: "supplydemand" as const,
                          label: "供需",
                          icon: TrendingUp,
                          level: PANEL_UNLOCK_CONFIG.supplydemand,
                        },
                      ] as const
                    )
                      .filter(
                        (tab) =>
                          !isPanelUnlocked(tab.id, gameState.cognition.level),
                      )
                      .map((tab) => (
                        <div
                          key={tab.id}
                  ...
                  <TabsContent value="operating" className="mt-6">
                    <OperatingPanel
                      ...
                      lastWeeklySummary={
                        gameState.lastWeeklySummary ?? undefined
                      }
                  ...
                  <TabsContent value="staff" className="mt-6">
                    <StaffPanel
                      ...
                      cognitionLevel={gameState.cognition.level}
                      storeArea={
                        gameState.selectedAddress?.area || gameState.storeArea
                      }
                      wageLevel={gameState.selectedLocation?.wageLevel || 1}
                      staffWorkStats={
                        (gameState.weeklySummary ?? gameState.lastWeeklySummary)
                          ?.staffWorkStats
                      }
            ...
            <CyberYongGe
              ...
              onConsult={() => {
                ...
                if (times >= PASSIVE_EXP_CONFIG.consultYongGeWeeklyLimit)
                  return false;
                if (gameState.cash < PASSIVE_EXP_CONFIG.consultYongGeCost)
                  return false;
                consultYongGe();
                return true;
              }}
