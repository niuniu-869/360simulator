...
/**
 * 生成全局唯一员工 ID。
 * 优先使用 crypto.randomUUID（所有现代浏览器 + Node 16+ 支持）；
 * fallback 用"毫秒 + 高熵随机串"保证同毫秒多次调用仍唯一。
 */
function generateStaffId(): string {
  if (
    ...
    return `staff_${crypto.randomUUID()}`;
  }
...
function requireOperatingPhase(
  prev: GameState,
  actionName: string,
): ActionResult | null {
...
const ACTIVE_OPERATION_TYPES = new Set([
  ...
  "toggle_promotion",
  "set_discount_tier",
  "set_delivery_pricing",
  "set_packaging_tier",
  "start_marketing",
  "stop_marketing",
  "set_product_price",
  "set_product_inventory",
  "set_restock_strategy",
  "assign_staff_task",
...
export function dispatch(state: GameState, action: GameAction): ActionResult {
  ...
  switch (action.type) {
    case "select_brand":
      result = handleSelectBrand(state, action.brandId);
      break;
    case "select_location":
      result = handleSelectLocation(state, action.locationId);
      break;
    case "select_address":
      result = handleSelectAddress(state, action.addressId);
      break;
    case "set_store_area":
      result = handleSetStoreArea(state, action.area);
      break;
    case "select_decoration":
      result = handleSelectDecoration(state, action.decorationId);
      break;
    case "toggle_product":
      result = handleToggleProduct(state, action.productId);
      break;
    case "add_staff":
      result = handleAddStaff(state, action.staffTypeId, action.assignedTask);
      break;
    case "fire_staff":
      result = handleFireStaff(state, action.staffId);
      break;
    case "open_store":
      result = handleOpenStore(state, action.season);
      break;
    case "next_week":
      result = handleNextWeek(state);
      break;
    case "restart":
      result = handleRestart();
      break;
    case "recruit_staff":
      result = handleRecruitStaff(
        state,
        action.channelId,
        action.staffTypeId,
        action.assignedTask,
      ...
      break;
    case "join_platform":
      result = handleJoinPlatform(state, action.platformId);
      break;
    case "leave_platform":
      result = handleLeavePlatform(state, action.platformId);
      break;
    case "toggle_promotion":
      result = handleTogglePromotion(
        state,
        action.platformId,
        action.tierIndex,
      );
    ...
    case "start_marketing":
      result = handleStartMarketing(state, action.activityId);
      break;
    case "stop_marketing":
      result = handleStopMarketing(state, action.activityId);
      break;
    case "set_product_price":
      result = handleSetProductPrice(state, action.productId, action.price);
      break;
    case "set_product_inventory":
      result = handleSetProductInventory(
        state,
        action.productId,
        action.quantity,
      );
      break;
    case "set_restock_strategy":
      result = handleSetRestockStrategy(
        state,
        action.productId,
        action.strategy,
      );
      break;
    case "assign_staff_task":
      result = handleAssignStaffTask(state, action.staffId, action.taskType);
      break;
    case "set_staff_work_hours":
      result = handleSetStaffWorkHours(
        state,
        action.staffId,
        action.days,
        action.hours,
      ...
      break;
    case "consult_yong_ge":
      result = handleConsultYongGe(state);
      break;
    case "clear_weekly_summary":
      result = handleClearWeeklySummary(state);
      break;
    case "clear_last_week_event":
      result = handleClearLastWeekEvent(state);
      break;
    // v2.7 员工系统升级
    case "set_staff_salary":
      result = handleSetStaffSalary(state, action.staffId, action.newSalary);
      break;
    case "staff_morale_action":
      result = handleStaffMoraleAction(
        state,
        action.actionType,
        action.targetStaffId,
        action.bonusAmount,
      );
      break;
    case "retain_staff":
      result = handleRetainStaff(state, action.staffId, action.method);
      break;
    // v2.8 产品专注度
    case "set_staff_focus_product":
      result = handleSetStaffFocusProduct(
        state,
        action.staffId,
        action.productId,
      );
      break;
    // v2.9 老板周行动
    case "set_boss_action":
      result = handleSetBossAction(
        state,
        action.action,
        action.role,
        action.shopId,
    ...
    case "respond_to_event":
      result = handleRespondToEvent(state, action.eventId, action.optionId);
      break;
    // 外卖运营 (v3.0)
    case "set_discount_tier":
      result = handleSetDiscountTier(state, action.platformId, action.tierId);
      break;
    case "set_delivery_pricing":
      result = handleSetDeliveryPricing(
        state,
        action.platformId,
        action.pricingId,
      );
      break;
    case "set_packaging_tier":
      result = handleSetPackagingTier(state, action.platformId, action.tierId);
      break;
    // 出餐分配优先级
    case "set_supply_priority":
      result = handleSetSupplyPriority(state, action.priority);
      break;
    default:
      return fail(
        state,
  ...
  if (setupEvent) {
    return ok({
      ...
      interactiveEventHistory: [
        ...newState.interactiveEventHistory,
        setupEvent.id,
      ],
    });
  }
...
function handleSelectLocation(
  ...
  if (prev.locationLocked)
    return fail(prev, "Location is locked by quick franchise");
  if (locationId === null)
    return ok({ ...prev, selectedLocation: null, selectedAddress: null });
...
function handleSelectAddress(
  ...
  const address = prev.selectedLocation.addresses.find(
    (a) => a.id === addressId,
  );
  ...
  const rentBase =
    ...
    (address.rentModifier || 1) *
    address.area;
  const nearbyShops = generateInitialShops(
    prev.selectedLocation.type,
    address.id,
    rentBase,
  );
  const consumerRings = generateConsumerRings(prev.selectedLocation, address);
  const assignedRings = assignNearbyShopsToConsumerRings(
    consumerRings,
    nearbyShops,
  );
  return ok({
    ...
    storeArea: address.area || prev.storeArea,
    nearbyShops,
    nearbyShopEvents: [],
    baseConsumerRings: consumerRings,
    consumerRings: assignedRings,
  });
}
...
function handleSelectDecoration(
  prev: GameState,
  ...
  const previousCost = prev.selectedDecoration
    ? prev.selectedDecoration.costPerSqm *
      prev.storeArea *
      (prev.decorationCostMarkup || 1)
    : 0;
  ...
  if (decorationId === null) {
    return ok({
      ...
      cash: prev.cash + previousCost,
      totalInvestment: prev.totalInvestment - previousCost,
    });
  }
  ...
  return ok({
    ...
    cash: prev.cash + previousCost - newCost,
    totalInvestment: prev.totalInvestment - previousCost + newCost,
    decorationLocked: isQF,
    decorationCostMarkup: costMarkup,
  });
}
...
function handleToggleProduct(prev: GameState, productId: string): ActionResult {
  ...
  if (
    allowed &&
    !exists &&
    !allowed.includes(product.category as "drink" | "food" | "snack" | "meal")
  ) {
  ...
  if (exists) {
    ...
    if (
      isOp &&
      prev.selectedProducts.length <= PRODUCT_ADJUSTMENT_CONFIG.minProducts
    )
      return fail(prev, "Min products reached");
    if (
      isOp &&
      (prev.weeklyProductChanges || 0) >=
        PRODUCT_ADJUSTMENT_CONFIG.maxWeeklyChanges
    )
      return fail(prev, "Weekly change limit");

    const newInvItems = isOp
      ? prev.inventoryState.items.filter((i) => i.productId !== productId)
      : prev.inventoryState.items;
    return ok({
      ...prev,
      selectedProducts: prev.selectedProducts.filter((p) => p.id !== productId),
      weeklyProductChanges: isOp
        ? (prev.weeklyProductChanges || 0) + 1
        : prev.weeklyProductChanges,
      inventoryState: isOp
        ? {
            ...prev.inventoryState,
            items: newInvItems,
            totalValue: newInvItems.reduce(
  ...
  if (prev.selectedProducts.length >= PRODUCT_ADJUSTMENT_CONFIG.maxProducts)
    return fail(prev, "Max products reached");
  if (
    isOp &&
    (prev.weeklyProductChanges || 0) >=
      PRODUCT_ADJUSTMENT_CONFIG.maxWeeklyChanges
    ...
    return fail(prev, "Weekly change limit");
  if (isOp && prev.cash < ADJUSTMENT_COSTS.addProduct.moneyCost)
    return fail(prev, "Not enough cash");
  ...
  return ok({
    ...
    productsLocked: isQF && newProducts.length >= 3,
    cash: newCash,
    weeklyProductChanges: isOp
      ? (prev.weeklyProductChanges || 0) + 1
      : prev.weeklyProductChanges,
    inventoryState: newInvState,
  });
}
...
function makeStaff(
  ...
  staffTypeId: string,
  isRecruit: boolean,
  channelQuality?: "normal" | "high" | "excellent",
  assignedTask?: string,
): Staff {
  ...
  if (staffType.payType === "hourly" && staffType.hourlyRate) {
    salary = Math.round(
      staffType.hourlyRate * defaultDays * defaultHours * 4 * wageLevel,
    );
  } else {
  ...
  const defaultTask =
    staffType.availableTasks.includes("chef") &&
    !staffType.availableTasks.includes("waiter")
      ? "chef"
      : "waiter";
  const initialTask =
    assignedTask && staffType.availableTasks.includes(assignedTask)
      ? assignedTask
      : defaultTask;
  ...
  return {
    ...
    workDaysPerWeek: defaultDays,
    workHoursPerDay: defaultHours,
    isOnboarding: isRecruit,
    onboardingEndsWeek: isRecruit ? prev.currentWeek + 1 : prev.currentWeek,
  };
}
...
function handleAddStaff(
  ...
  if (!staffTypes.find((s) => s.id === staffTypeId))
    return fail(prev, `Staff type not found: ${staffTypeId}`);
  if (!prev.selectedLocation) return fail(prev, "No location selected");
  const MAX_STAFF_SETUP = 8;
  if (prev.staff.length >= MAX_STAFF_SETUP)
    return fail(prev, "筹备阶段最多招聘8人");
  return ok({
    ...prev,
    staff: [
      ...prev.staff,
      makeStaff(prev, staffTypeId, false, undefined, assignedTask),
    ],
  });
}
...
function handleFireStaff(prev: GameState, staffId: string): ActionResult {
  ...
  const remaining = prev.staff
    .filter((s) => s.id !== staffId)
    .map((s) => ({
      ...s,
      morale: Math.max(0, Math.min(100, s.morale + moralePenalty)),
    }));
...
function handleRecruitStaff(
  ...
  const newStaff = makeStaff(
    ...
    staffTypeId,
    true,
    channel.candidateQuality,
    assignedTask,
  );
  return ok({
    ...prev,
    staff: [...prev.staff, newStaff],
    cash: prev.cash - channel.cost,
...
function handleJoinPlatform(prev: GameState, platformId: string): ActionResult {
  ...
  return ok({
    ...
    deliveryState: {
      ...prev.deliveryState,
      platforms,
      totalPlatformExposure: platforms.reduce(
...
function handleLeavePlatform(
  ...
  const platforms = prev.deliveryState.platforms.filter(
    (p) => p.platformId !== platformId,
  );
  return ok({
    ...prev,
    hasDelivery: platforms.length > 0,
    deliveryState: {
      ...prev.deliveryState,
      platforms,
      totalPlatformExposure: platforms.reduce(
  ...
  const platforms = prev.deliveryState.platforms.map((p) =>
    p.platformId !== platformId
      ...
      : {
          ...
          weeklyPromotionCost: tier.weeklyCost,
        },
  );
  return ok({
    ...prev,
    deliveryState: { ...prev.deliveryState, platforms },
    cognition: bumpOps(prev),
  });
}

function handleSetDiscountTier(
  prev: GameState,
  platformId: string,
  tierId: string,
): ActionResult {
...
function handleSetDeliveryPricing(
  prev: GameState,
  platformId: string,
  pricingId: string,
): ActionResult {
...
function handleSetPackagingTier(
  prev: GameState,
  platformId: string,
  tierId: string,
): ActionResult {
...
function handleOpenStore(prev: GameState, season?: Season): ActionResult {
  ...
  const weeklyRent =
    ((prev.selectedLocation?.rentPerSqm || 0) * area * rentModifier) / 4;
...
function handleStartMarketing(
  ...
  if (prev.activeMarketingActivities.some((a) => a.id === activityId))
    return fail(prev, "Already active");
  if (config.unique && prev.usedOneTimeActivities.includes(activityId))
    return fail(prev, "Already used");

  if (config.cooldownWeeks && config.type === "one_time") {
    const last = prev.lastActivityWeek[activityId];
    if (last !== undefined && prev.currentWeek - last < config.cooldownWeeks)
      return fail(prev, "In cooldown");
  }
  if (config.type === "one_time" && prev.cash < config.baseCost)
    return fail(prev, "Not enough cash");
  ...
  const act: MarketingActivity = {
    ...
    weeklyCost: config.baseCost,
    exposureBoost: config.exposureBoost,
    reputationBoost: config.reputationBoost,
    priceModifier: config.priceModifier,
    dependencyCoefficient: config.dependencyCoefficient,
    activeWeeks: 0,
  ...
  return ok({
    ...
    usedOneTimeActivities:
      config.type === "one_time"
        ? [...prev.usedOneTimeActivities, activityId]
        : prev.usedOneTimeActivities,
    lastActivityWeek:
      config.type === "one_time"
        ? { ...prev.lastActivityWeek, [activityId]: prev.currentWeek }
        : prev.lastActivityWeek,
    cognition: bumpOps(prev),
  });
}

function handleStopMarketing(
  prev: GameState,
  activityId: string,
): ActionResult {
  const phaseGuard = requireOperatingPhase(prev, "营销活动");
  if (phaseGuard) return phaseGuard;
  const activity = prev.activeMarketingActivities.find(
    (a) => a.id === activityId,
  );
  if (!activity) return fail(prev, `Activity not found: ${activityId}`);
  const penalty = calculateStopPenalty(
    activity.dependencyCoefficient,
    activity.activeWeeks,
  );
  const floor = getLocationExposureFloor(
    prev.selectedAddress?.trafficModifier || 1,
  );
  return ok({
    ...prev,
    activeMarketingActivities: prev.activeMarketingActivities.filter(
      (a) => a.id !== activityId,
    ),
...
function handleSetProductPrice(
  prev: GameState,
  productId: string,
  price: number,
): ActionResult {
  ...
  const clampedPrice = Math.max(
    1,
    Math.min(maxPrice, Math.round(price * 100) / 100),
  );
  return ok({
    ...prev,
...
function handleSetProductInventory(
  prev: GameState,
  productId: string,
  quantity: number,
): ActionResult {
  ...
  if (phaseGuard) return phaseGuard;
  if (prev.cognition.level < 1)
    return fail(prev, "Cognition level too low for inventory ops");
  if (!Number.isFinite(quantity)) return fail(prev, "库存数量必须为有限数值");
  const product = prev.selectedProducts.find((p) => p.id === productId);
  if (!product) return fail(prev, `Product not found: ${productId}`);

  const unitCost = product.baseCost * getEffectiveSupplyCostModifier(prev);
  const existing = prev.inventoryState.items.find(
  ...
  if (existing) {
    ...
    newItems = [
      ...prev.inventoryState.items,
      {
        ...
        quantity: newQty,
        unitCost,
        storageType: product.storageType as
          | "normal"
          | "refrigerated"
          | "frozen",
        restockStrategy: "auto_standard" as const,
        lastWeekSales: 0,
        lastWeekWaste: 0,
        lastRestockQuantity: newQty,
        lastRestockCost: newQty * unitCost,
      },
    ];
  }
...
function handleSetRestockStrategy(
  prev: GameState,
  productId: string,
  strategy: import("@/types/game").RestockStrategy,
): ActionResult {
...
function handleAssignStaffTask(
  prev: GameState,
  staffId: string,
  taskType: string,
): ActionResult {
  ...
  if (staff.assignedTask === taskType)
    return fail(prev, "Already on this task");
    ...
    staff: prev.staff.map((s) =>
      s.id === staffId
        ? {
            ...
            transitionEndsWeek:
              prev.currentWeek + TRANSITION_CONFIG.durationWeeks,
          }
        : s,
    ),
    cognition: bumpOps(prev),
...
function handleSetStaffWorkHours(
  prev: GameState,
  staffId: string,
  days: number,
  hours: number,
  ...
  if (staffType?.payType === "hourly" && staffType.hourlyRate) {
    ...
    newSalary = Math.round(
      staffType.hourlyRate * clampedDays * clampedHours * 4 * wageLevel,
    );
  }
  ...
  return ok({
    ...
    staff: prev.staff.map((s) =>
      s.id === staffId
        ? {
            ...s,
            workDaysPerWeek: clampedDays,
            workHoursPerDay: clampedHours,
            salary: newSalary,
          }
        : s,
    ),
...
function handleConsultYongGe(prev: GameState): ActionResult {
  ...
  if (times >= PASSIVE_EXP_CONFIG.consultYongGeWeeklyLimit)
    return fail(prev, "Weekly consult limit reached");
  if (prev.cash < PASSIVE_EXP_CONFIG.consultYongGeCost)
    return fail(prev, "Not enough cash");

  // 统一认知升级入口
  const newCognition = applyCognitionExp(
    { ...prev.cognition },
    PASSIVE_EXP_CONFIG.consultYongGeExp,
  );
...
function handleSetStaffSalary(
  ...
  if (phaseGuard) return phaseGuard;
  if (prev.cognition.level < 2)
    return fail(prev, "认知等级不足，需要达到觉醒新手(Lv2)才能调整薪资");
  ...
  // 时薪制员工不支持手动调薪
  if (staffType.payType === "hourly")
    return fail(prev, "兼职员工按时薪计费，请通过调整工时来控制成本");
  ...
  const clampedSalary = Math.max(
    minSalary,
    Math.min(maxSalary, Math.round(newSalary)),
  );
  ...
  if (isRaise) {
    // 加薪：即时士气提升 + 持续加成（每周衰减）
    moraleChange = Math.min(
      SALARY_CONFIG.raiseMaxBoost,
      Math.round((changeRatio * 100 * SALARY_CONFIG.raiseCoefficient) / 100),
    );
    salaryRaiseMoraleBoost = moraleChange;
  } else {
    // 降薪：士气暴跌
    moraleChange = -Math.round(
      (changeRatio * 100 * SALARY_CONFIG.cutCoefficient) / 100,
    );
  }
  ...
  const updatedStaff = prev.staff.map((s) => {
    if (s.id === staffId) {
      ...
      return {
        ...
        morale: newMorale,
        salaryRaiseMoraleBoost: isRaise
          ? salaryRaiseMoraleBoost
...
function handleStaffMoraleAction(
  ...
  if (prev.cognition.level < config.minCognitionLevel) {
    return fail(
      prev,
      "认知等级不足，需要达到踩坑学徒(Lv1)才能使用士气管理工具",
    );
  }
  ...
  if (actionType === "bonus") {
    ...
    // 冷却检查
    if (
      staff.lastBonusWeek !== undefined &&
      prev.currentWeek - staff.lastBonusWeek < config.cooldownWeeks
    ) {
      return fail(
        prev,
        `奖金冷却中，还需${config.cooldownWeeks - (prev.currentWeek - staff.lastBonusWeek)}周`,
      );
    }
    ...
    const updatedStaff = prev.staff.map((s) => {
      if (s.id === targetStaffId) {
        return {
          ...s,
          morale: Math.min(100, s.morale + moraleBoost),
          lastBonusWeek: prev.currentWeek,
        };
      }
      // 其他员工公平感加成
  ...
  if (actionType === "team_meal") {
    // 全局冷却检查
    if (
      prev.lastTeamMealWeek !== undefined &&
      prev.currentWeek - prev.lastTeamMealWeek < config.cooldownWeeks
    ) {
      return fail(
        prev,
        `团建冷却中，还需${config.cooldownWeeks - (prev.currentWeek - prev.lastTeamMealWeek)}周`,
      );
    }
  ...
  if (actionType === "day_off") {
    ...
    if (
      staff.lastDayOffWeek !== undefined &&
      prev.currentWeek - staff.lastDayOffWeek < config.cooldownWeeks
    ) {
      return fail(
        prev,
        `放假冷却中，还需${config.cooldownWeeks - (prev.currentWeek - staff.lastDayOffWeek)}周`,
      );
    }
...
function handleRetainStaff(
  ...
  if (method === "raise") {
    const newSalary = Math.round(
      staff.salary * (1 + RETENTION_CONFIG.raise.salaryIncreaseRate),
    );
    if (success) {
      const updatedStaff = prev.staff.map((s) =>
        s.id === staffId
          ? {
              ...
              morale: Math.min(
                100,
                s.morale + RETENTION_CONFIG.raise.moraleBoost,
              ),
              wantsToQuit: false,
            }
          : s,
        ...
        s.id === staffId
          ? {
              ...
              fatigue: Math.max(
                ...
                s.fatigue -
                  (
                    RETENTION_CONFIG.reduce_hours as {
  ...
  if (method === "bonus") {
    const bonusCost = Math.round(
      staff.salary * RETENTION_CONFIG.bonus.costRatio,
    );
      ...
      const updatedStaff = prev.staff.map((s) =>
        s.id === staffId
          ? {
              ...
              morale: Math.min(
                100,
                s.morale + RETENTION_CONFIG.bonus.moraleBoost,
              ),
              wantsToQuit: false,
            }
          : s,
...
function handleSetStaffFocusProduct(
  prev: GameState,
  staffId: string,
  ...
  if (
    staffType &&
    !staffType.canHandleProducts.includes(
      product.category as "drink" | "food" | "snack" | "meal",
    )
  ) {
...
function handleSetBossAction(
  prev: GameState,
  actionType: BossActionType,
  role?: string,
  shopId?: string,
): ActionResult {
  if (prev.gamePhase !== "operating")
    return fail(prev, "仅经营阶段可设置老板行动");
  ...
  if (
    prev.bossAction.currentAction === actionType &&
    actionType !== "work_in_store" &&
    !isShopUpdate
  ) {
    return ok({
      ...prev,
  ...
  if (actionType === "work_in_store") {
    ...
    newBossAction.workRole =
      role && validRoles.includes(role) ? role : "waiter";
  } else {
...
function handleSetSupplyPriority(
  prev: GameState,
  priority: SupplyPriority,
): ActionResult {
  if (prev.gamePhase !== "operating")
    return fail(prev, "仅经营阶段可设置出餐分配");
...
function handleRespondToEvent(
  ...
  if (isNotificationEvent(pending) && optionId === "__notification__") {
    const stateAfterEffects = applyEventEffects(
      prev,
      pending.notificationEffects!,
      eventId,
    );
    return ok({
      ...stateAfterEffects,
