...
const CONTEXT_CHECKS: Record<string, ContextCheckFn> = {
  ...
  deep_loss: (s) => {
    return (
      s.totalInvestment > 0 && s.cumulativeProfit < -(s.totalInvestment * 0.4)
    );
  },
  ...
  high_fatigue: (s) => {
    ...
    const avgFatigue =
      s.staff.reduce((sum, st) => sum + st.fatigue, 0) / s.staff.length;
  ...
  // 供给满足率 < 70%
  supply_shortage: (s) => {
    return s.lastWeekFulfillment < 0.7;
  },
...
export function rollInteractiveEvent(
  ...
  for (const event of INTERACTIVE_EVENTS) {
    ...
    if (event.triggerCondition.phase === "setup") {
      if (
        event.triggerCondition.setupStep &&
        event.triggerCondition.setupStep !== setupStep
      )
        continue;
    ...
    // 周次范围
    if (
      event.triggerCondition.minWeek !== undefined &&
      currentWeek < event.triggerCondition.minWeek
    )
      continue;
    if (
      event.triggerCondition.maxWeek !== undefined &&
      currentWeek > event.triggerCondition.maxWeek
    )
      continue;
  ...
  switch (selector) {
    case "highest_skill":
      return candidates.reduce(
        (best, s) => (s.skillLevel > best.skillLevel ? s : best),
        candidates[0],
      );
    case "lowest_morale":
      return candidates.reduce(
        (best, s) => (s.morale < best.morale ? s : best),
        candidates[0],
      );
    case "highest_fatigue":
      return candidates.reduce(
        (best, s) => (s.fatigue > best.fatigue ? s : best),
        candidates[0],
      );
    case "random":
...
export function applyEventEffects(
  ...
  if (effects.reputation) {
    newState.reputation = Math.max(
      0,
      Math.min(100, newState.reputation + effects.reputation),
    );
  }

  if (effects.exposure) {
    newState.exposure = Math.max(
      0,
      Math.min(100, newState.exposure + effects.exposure),
    );
  }
  ...
  if (effects.cleanliness) {
    newState.cleanliness = Math.max(
      0,
      Math.min(100, (newState.cleanliness ?? 60) + effects.cleanliness),
    );
  }
  ...
  if (effects.cognitionExp) {
    newState.cognition = applyCognitionExp(
      newState.cognition,
      effects.cognitionExp,
    );
  }
    ...
    if (target) {
      if (staffEffects.remove) {
        ...
        newState.staff = newState.staff.map((s) => {
          ...
          const updated = { ...s };
          if (staffEffects.morale)
            updated.morale = Math.max(
              0,
              Math.min(100, updated.morale + staffEffects.morale),
            );
          if (staffEffects.fatigue)
            updated.fatigue = Math.max(
              0,
              Math.min(100, updated.fatigue + staffEffects.fatigue),
            );
          if (staffEffects.salary)
            updated.salary = Math.max(0, updated.salary + staffEffects.salary);
          if (staffEffects.wantsToQuit !== undefined)
            updated.wantsToQuit = staffEffects.wantsToQuit;
          return updated;
        });
      }
    }
