...
import { CONVERSION_RATE, SEASON_MODIFIER } from "@/data/gameData";
...
// 修复 #8：邻环权重不对称
const INNER_TO_OUTER_WEIGHT = 0.6; // 近处好店吸引远处消费者（顺路效应强）
const OUTER_TO_INNER_WEIGHT = 0.4; // 远处店铺对近处消费者也有一定威胁
...
function getRingShopIdsFromState(state: GameState, ringId: RingId): string[] {
  ...
  return (state.nearbyShops || [])
    .filter((s) => s.ring === ringId)
    .map((s) => s.id);
}

function getWeightedCompetitorsForRing(
  state: GameState,
  ringId: RingId,
): Array<{ shop: NearbyShop; weight: number }> {
  ...
  getAdjacentRings(ringId).forEach((adj) => {
    ...
    const weight =
      adjIdx < ringIdx ? INNER_TO_OUTER_WEIGHT : OUTER_TO_INNER_WEIGHT;
    addRing(adj, weight);
  });
 ...
 * - 价格 ≈ referencePrice (0.8~1.25): 1.0（合理区间，含25%溢价空间）
 * - 价格 > referencePrice*1.25: 1.8 次方幂函数惩罚（更贴近现实消费者行为）
 *
 * 校准：原三次方惩罚过度（ratio=2.0→0.24），与"蜜雪4元/喜茶20元共存"等
 * 中国真实市场消费者对溢价品牌容忍度不符。改为 1.8 次方，ratio=1.5→0.73,
 * ratio=2.0→0.44，保持惩罚但不再"一溢价就崩盘"。
 */
export function calculateAbsolutePriceEffect(
  ...
  // 超出合理溢价：1.8 次方幂函数惩罚（原 3.0 过度）
  // 例：ratio=1.3→0.93, ratio=1.5→0.73, ratio=2.0→0.44, ratio=3.0→0.21
  return Math.max(0.08, Math.pow(1.25 / ratio, 1.8));
}
...
const LOCATION_CATEGORY_OCCASION: Record<
  string,
...
// 客群-品类偏好修正
const CUSTOMER_CATEGORY_AFFINITY: Record<
  CustomerType,
  Record<Product["category"], number>
> = {
...
const CATEGORY_SUBSTITUTION_MATRIX: Record<
  Product["category"],
  Partial<Record<NearbyShop["shopCategory"], number>>
> = {
 ...
 * 获取产品在当前季节的加成系数
 */
function getSeasonSubTypeBonus(
  season: Season,
...
function getCategoryOccasionMultiplier(
  ...
  const locationFactor =
    LOCATION_CATEGORY_OCCASION[locationType || "community"]?.[category] ?? 1.0;
  const customerFactor =
    CUSTOMER_CATEGORY_AFFINITY[customerType]?.[category] ?? 1.0;
  return clamp(locationFactor * customerFactor, 0.75, 1.35);
}

function getMenuBalanceFactor(
  products: Product[],
  category: Product["category"],
): number {
...
export function getTrafficReachMultiplier(
  exposure: number,
  reputation: number,
): number {
  const exposureBoost = Math.min(
    0.28,
    (Math.max(0, exposure - 10) / 90) * 0.28,
  );
  const reputationBoost = Math.min(
    0.14,
    (Math.max(0, reputation - 30) / 70) * 0.14,
  );
...
export function getNewStoreAwarenessFactor(
  ...
  if (growthSystem?.launchProgress !== undefined) {
    const normalized =
      1 / (1 + Math.exp(-(growthSystem.launchProgress - 45) / 10));
  ...
  if (brandType === "franchise") {
    ...
    return 0.3 + 0.7 * (currentWeek / rampWeeks);
  }
  ...
  const rampWeeks = 12;
  if (currentWeek >= rampWeeks) return 1.0;
  return 0.2 + 0.8 * (currentWeek / rampWeeks);
}
...
export function calculateAttractionScore(state: GameState): number {
  ...
  return Math.min(
    100,
    decorationScore + weekBonus + reputationScore + offlineExposureScore,
  );
}
 ...
 * 吸引力分数 → 各环覆盖率
 */
export function getRingCoverage(
...
export function calculateDemandModifiers(state: GameState): DemandModifiers {
  ...
  if (state.staff.length > 0) {
    ...
    if (serviceStaff.length === 0) {
      ...
      const avgServiceQuality =
        serviceStaff.reduce((sum, s) => sum + (s.serviceQuality || 0.8), 0) /
        serviceStaff.length;
  ...
  const reputationProtection = Math.min(
    0.1,
    (Math.max(0, state.reputation - 60) / 40) * 0.1,
  );
  const inactivityContribution = -Math.max(
    0,
    basePenalty - reputationProtection,
  );

  // 4. 整洁度贡献（与服务质量并列的独立修正因子）
  const cleanlinessContribution =
    (((state.cleanliness ?? 60) - 50) / 50) * 0.08;
...
function calculatePlayerCompetitiveness(
  ...
  const reputationProtection = Math.min(
    0.2,
    (Math.max(0, playerReputation - 60) / 40) * 0.2,
  );
  const operationHeatScore =
    1 - Math.max(0, heatPenalty - reputationProtection);
...
function calculateShopCompetitiveness(
  ...
  const sameCatProducts = shop.products.filter(
    (p) => p.category === productCategory,
  );
  const relevantProducts =
    sameCatProducts.length > 0 ? sameCatProducts : shop.products;
  const avgPrice =
    relevantProducts.length > 0
      ? relevantProducts.reduce((s, p) => s + p.price, 0) /
        relevantProducts.length
      : 10;
  const avgAppeal =
    relevantProducts.length > 0
      ? relevantProducts.reduce((s, p) => s + p.appeal, 0) /
        relevantProducts.length
      : 50;
  ...
  return (
    priceScore * exposureScore * reputationScore * qualityScore * serviceScore
  );
}
...
function calculateCategorySubstitutionPressure(
  ...
  const diversionScore = weightedShops.reduce((sum, entry) => {
    ...
    const strength =
      (entry.shop.exposure / 100) * Math.max(0.3, entry.shop.serviceQuality);
...
export function calculateDemand(state: GameState): DemandBreakdown {
  ...
  const trafficReachMultiplier = getTrafficReachMultiplier(
    state.exposure,
    state.reputation,
  );
  ...
  const totalBaseTraffic = Object.values(baseTraffic).reduce(
    (sum, v) => sum + v,
    0,
  );
  ...
  const productDemands: ProductDemandBreakdown[] = products.map((product) => {
    ...
    // 绝对价格弹性（基于消费者心理价位）
    const absolutePriceEffect = calculateAbsolutePriceEffect(
      actualPrice,
    ...
    // 玩家产品吸引力
    const avgAppeal =
      (product.appeal.students +
        product.appeal.office +
        product.appeal.family +
        product.appeal.tourist) /
      4;
    ...
    const ringsToProcess = hasRings
      ...
      : [
          {
            distance: "ring0" as const,
            consumers: baseTraffic,
            baseConversion: 1.0,
            label: "",
            nearbyShopIds: [],
          },
    ...
    ringsToProcess.forEach((ring) => {
      ...
      const weightedCompetitors = getWeightedCompetitorsForRing(
        state,
        ring.distance,
      );
      ...
      weightedDirectCompetitors.forEach((entry) => {
        const sameCatProducts = entry.shop.products.filter(
          (p) => p.category === product.category,
        );
        if (sameCatProducts.length === 0) return; // 该店无同品类产品，不参与均价
        const shopCatAvgPrice =
          sameCatProducts.reduce((s, p) => s + p.price, 0) /
          sameCatProducts.length;
      ...
      CUSTOMER_TYPES.forEach((type) => {
        const ringConsumers =
          ring.consumers[type] *
          (ring.distance === "ring0" ? 1 : trafficReachMultiplier);
        if (ringConsumers <= 0) return;

        const appealRatio = calculateProductAppealRatio(
          product,
          products,
          state,
          type,
        ...
        const menuBalanceFactor = getMenuBalanceFactor(
          products,
          product.category,
        );
        ...
        const effectiveConsumers =
          ringConsumers * coverage * ring.baseConversion * brandTraffic;
        const areaDemand =
          effectiveConsumers *
        ...
        const marketShare =
          playerComp / (playerComp + shopCompTotal + BASE_MARKET_FRICTION);

        const playerModifier =
          modifiers.season +
          modifiers.marketing +
          modifiers.serviceQuality +
          modifiers.cleanliness +
          modifiers.inactivity;
    ...
    const awarenessFactor = getNewStoreAwarenessFactor(
      state.currentWeek,
      brandType,
      state.growthSystem,
    );
    ...
    const varianceSeed =
      (hashStringToUint32(product.id) ^
        Math.imul(state.currentWeek, 2654435761)) >>>
      0;
    const varianceFactor =
      1 + (rand01FromSeed(varianceSeed) - 0.5) * 2 * WEEKLY_DEMAND_VARIANCE;
    let weeklyDemand =
      totalProductDemand * 7 * varianceFactor * awarenessFactor;
    ...
    return {
      ...
      baseTrafficByType,
      appealRatio: calculateProductAppealRatio(
        product,
        products,
        state,
        "students",
  ...
  const totalDemand = productDemands.reduce(
    (sum, pd) => sum + pd.finalDemand,
    0,
  );
