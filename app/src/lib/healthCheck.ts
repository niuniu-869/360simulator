...
export function diagnoseHealth(
  ...
  // 1. chronic_loss: 连续2周亏损（原3周，现实中经营者1-2周就需警觉）
  if (state.profitHistory.length >= 2) {
    const last2 = state.profitHistory.slice(-2);
    if (last2.every((p) => p < 0)) {
      alerts.push({
        id: "chronic_loss",
        severity: "critical",
        title: "连续亏损警报！",
        message:
          "哥们，你已经连续2周亏钱了，再这样下去现金流要断了。不要跟我说感觉，给我看数据！",
        suggestion:
          "立刻检查固定成本结构，考虑裁减冗余人员或停止低效营销活动。",
  ...
  // 2. slow_bleeding: 微利陷阱（利润为正但<固定成本的20%）
  // 原阈值 10% 过低——餐饮行业毛利微薄时，任何意外都可能翻盘。
  // 按业内经验，利润覆盖固定成本 20% 才算安全边际（单月租金涨幅、员工离职成本 ≈ 20%）。
  if (
    stats.profit > 0 &&
    stats.fixedCost > 0 &&
    stats.profit < stats.fixedCost * 0.2
  ) {
    alerts.push({
      id: "slow_bleeding",
      severity: "warning",
      title: "微利陷阱",
      message:
        "看着是赚钱了，但这点利润连固定成本的零头都不够。一个意外就能把你打回亏损。",
      suggestion:
        "提升客单价或增加高毛利产品，目标是利润覆盖固定成本的30%以上。",
      category: "finance",
    });
  }
  ...
  if (state.exposure < 25 && state.activeMarketingActivities.length === 0) {
    const hasMarketer = state.staff.some(
      (s) => s.assignedTask === "marketer" && !s.isOnboarding,
    );
    alerts.push({
      id: "low_exposure",
      severity: "warning",
  ...
  if (
    ...
    alerts.push({
      ...
      title: "没人帮你拉客",
      message:
        "既没有营销员也没有营销活动，曝光度全靠自然衰减，知名度只会越来越低。",
  ...
  if (state.reputation < 40 && state.currentWeek > 4) {
    alerts.push({
      ...
      title: "口碑堪忧",
      message:
        "回头客太少了，说明顾客体验有问题。口碑上不去，做再多推广也是白搭。",
  ...
  if (sdResult && sdResult.demand.totalDemand > 0) {
    ...
    const actualFulfillment =
      sdResult.supply.totalSupply > 0
        ? Math.min(
            1,
            sdResult.totalSales / Math.max(1, sdResult.demand.totalDemand),
          )
        : 0;
    if (
      sdResult.supply.totalSupply > 0 &&
      ...
      alerts.push({
        ...
        title: "门可罗雀",
        message:
          "供给能力远超需求，员工闲着没事干。要么是位置不行，要么是没人知道你。",
  ...
  if (state.staff.length > 0) {
    ...
    if (
      stats.revenue <= 0 ? weeklySalary > 0 : weeklySalary > stats.revenue * 0.6
    ) {
      alerts.push({
        ...
        message:
          "你在做慈善呢？周工资" +
          fuzzMoney(weeklySalary, cl) +
          "，周收入才" +
          fuzzMoney(stats.revenue, cl) +
          "，人工占比超过60%了。",
  ...
  const hasMealProduct = state.selectedProducts.some(
    (p) => p.category === "meal",
  );
  ...
  if (
    ...
    state.inventoryState.weeklyWasteCost > stats.variableCost * 0.2
  ) {
    alerts.push({
      id: "high_waste",
      severity: "warning",
      ...
      message:
        "库存损耗占变动成本的" +
        fuzzPercent(
          (state.inventoryState.weeklyWasteCost / stats.variableCost) * 100,
          cl,
        ) +
        "，东西都烂在仓库里了。",
  ...
  if ((state.cleanliness ?? 60) < 40) {
    const hasCleaner = state.staff.some(
      (s) => s.assignedTask === "cleaner" && !s.isOnboarding,
    );
    alerts.push({
      id: "low_cleanliness",
      severity: "warning",
      ...
      if (s.skillLevel >= 3 && s.morale < 50) {
        ...
        const avgMorale =
          state.staff.length > 0
            ? state.staff.reduce((sum, st) => sum + st.morale, 0) /
              state.staff.length
            : 50;
  ...
  if (state.staff.length > 0) {
    const avgMorale =
      state.staff.reduce((sum, s) => sum + s.morale, 0) / state.staff.length;
    if (avgMorale < 40 && (state.weeksSinceLastMoraleAction || 99) >= 4) {
      alerts.push({
        ...
        suggestion:
          "立刻组织团建聚餐提振士气，或给表现好的员工发奖金。认知达到Lv1即可使用士气管理工具。",
        category: "staff",
      });
    }
  }
  ...
  if (state.staff.length > 0) {
    const exhaustedCount = state.staff.filter(
      (s) => !s.isOnboarding && s.fatigue > 70,
    ).length;
    ...
    if (activeCount > 0 && exhaustedCount === activeCount) {
      alerts.push({
        ...
        title: "全员过劳！",
        message:
          "所有员工疲劳度都超过70%了，效率大打折扣，离职风险极高。再不休息就要集体罢工了。",
  ...
  if (state.deliveryState.platforms.length > 0) {
    const allNoDiscount = state.deliveryState.platforms.every(
    ...
    if (allNoDiscount && state.currentWeek > 2) {
      alerts.push({
        ...
        title: "外卖没做满减！",
        message:
          "上了外卖但一个满减活动都没设，平台会严重降权，几乎没有自然流量。现在做外卖不做满减等于白上。",
        suggestion:
          '至少设置"小额满减"（满20减3），标准满减效果更好但补贴成本更高，根据毛利率选择。',
        category: "delivery",
      });
    }
  }
  ...
  if (state.deliveryState.platforms.length > 0) {
    ...
    if (lossLeaderPlatforms.length > 0) {
      alerts.push({
        ...
        title: "亏本冲量不能长干！",
        message:
          "满减力度太大了，每单都在亏钱补贴顾客。冲量可以短期做，长期做就是给平台打工。",
        suggestion:
          '单量稳定后切换到"标准满减"或"小额满减"，靠口碑和复购维持单量。',
        category: "delivery",
      });
    }
  }
  ...
  if (
    state.cumulativeProfit < -(state.totalInvestment * 0.5) &&
    state.currentWeek > 6 &&
    ...
    alerts.push({
      ...
      title: "沉没成本陷阱！",
      message:
        '已经亏了总投资的一半以上了，你还在想"再试试"？越陷越深只会亏得更多。',
      suggestion:
        "冷静算一笔账：按目前趋势还要亏多久？如果看不到扭亏的具体路径，及时止损比硬撑更明智。",
  ...
  if (state.selectedBrand?.isQuickFranchise) {
    ...
    if (state.currentWeek >= hw - 1 && state.currentWeek <= hw) {
      alerts.push({
        ...
        title: "蜜月期快结束了…",
        message:
          '快招品牌的"扶持期"马上到期，之后供货成本会暴涨，总部刷的好评也会消失。做好心理准备。',
        suggestion:
          "提前储备现金，考虑寻找替代供应商或调整产品结构降低对总部供货的依赖。",
    ...
    if (state.currentWeek === hw + 1 || state.currentWeek === hw + 2) {
      alerts.push({
        ...
        title: "蜜月期结束了！",
        message:
          '总部的"扶持"已经撤了，供货成本回到真实水平，口碑加成也没了。接下来才是真正的考验。',
        suggestion:
          "重新核算盈亏平衡点，如果发现根本不赚钱，趁早考虑转型或止损。",
        relatedCaseId: "naza-drink",
        category: "finance",
      });
    }
  ...
  if (
    stats.breakEvenPoint > 0 &&
    stats.breakEvenPoint < Infinity &&
    stats.revenue < stats.breakEvenPoint * 0.5 &&
    state.currentWeek > 3
  ) {
    alerts.push({
      ...
      title: "离保本还远着呢",
      message:
        "你的营业额连盈亏平衡点的一半都不到。简单说：每开一天都在亏钱，而且亏得不少。",
      suggestion:
        "盈亏平衡点 = 固定成本 ÷ 毛利率。要么想办法把营业额翻倍，要么砍固定成本。两头都动不了就该考虑止损了。",
      category: "finance",
    });
  }
