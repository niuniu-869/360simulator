...
export function getBossActionConfig(
  actionId: BossActionType,
): BossActionConfig | undefined {
...
/** 老板替代员工时的效率折扣 */
export const BOSS_WORK_EFFICIENCY = 0.7;
...
export function generateInvestigationResult(
  ...
  const isAccurate = rand() < INVESTIGATION_ACCURACY[cogLevel];
  ...
  switch (dimension) {
    ...
    case "price": {
      const avgPrice =
        shop.products.length > 0
          ? Math.round(
              shop.products.reduce((s, p) => s + p.price, 0) /
                shop.products.length,
            )
          : 20;
      realValue = `¥${avgPrice}`;
      const fakeOffset = Math.round(
        (3 + rand() * 10) * (rand() > 0.5 ? 1 : -1),
      );
    ...
    case "category": {
      ...
      const fakeCategories = Object.values(categoryNames).filter(
        (c) => c !== realValue,
      );
      fakeValue = fakeCategories[Math.floor(rand() * fakeCategories.length)];
      break;
    }
    case "decoration": {
      ...
      fakeValue = fakeLevels[Math.floor(rand() * fakeLevels.length)];
      break;
    }
    case "staffCount": {
      // 基于店铺品牌类型估算员工数
      ...
      const fakeCount = Math.max(1, realCount + Math.round(rand() * 4 - 2));
      fakeValue =
        fakeCount === realCount ? `${realCount + 2} 人` : `${fakeCount} 人`;
      break;
    }
  }
  ...
  if (!isAccurate && cogLevel >= COG_WARNING_THRESHOLD) {
    cogWarning =
      COG_WARNING_MESSAGES[Math.floor(rand() * COG_WARNING_MESSAGES.length)];
  }
...
export const DINNER_INSIGHTS = {
  accurate: [
    {
      content: "听说最近{category}品类竞争加剧，新店开了好几家",
      category: "market",
    },
    {
      content: "有个老板说他家用的供应商比市场价便宜10%，在城东批发市场",
      category: "supply",
    },
    {
      content: "这片区周末客流比工作日多40%左右，得备足周末的货",
      category: "demand",
    },
    { content: "隔壁街那家店上了外卖后营业额涨了30%", category: "delivery" },
    {
      content: "现在招人不好招，得把薪资开到行业均价以上才行",
      category: "staff",
    },
  ...
  inaccurate: [
    ...
    {
      content: "有人推荐了个供应商，说能便宜30%，质量一样好",
      category: "supply",
    },
...
export function generateDinnerInsight(
  ...
  const isAccurate = rand() < DINNER_RELIABILITY[cogLevel];

  const pool = isAccurate
    ? DINNER_INSIGHTS.accurate
    : DINNER_INSIGHTS.inaccurate;
  const insight = pool[Math.floor(rand() * pool.length)];
  ...
  const categoryName = playerCategory
    ? categoryNames[playerCategory] || playerCategory
    : "餐饮";
  ...
  if (
    isAccurate &&
    insight.category === "supply" &&
    rand() < DINNER_BUFF_CHANCE / DINNER_RELIABILITY[cogLevel]
  ) {
    buff = {
      type: "supply_cost_reduction",
      value: 0.05, // 进货成本 -5%
