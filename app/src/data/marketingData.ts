...
import {
  SOCIAL_MEDIA_WEEKLY_COST,
  INGREDIENT_UPGRADE_WEEKLY_COST,
} from "@/data/balance";
...
export const EXPOSURE_ACTIVITIES: MarketingActivityConfig[] = [
  {
    ...
    type: "continuous",
    category: "exposure",
    baseCost: SOCIAL_MEDIA_WEEKLY_COST, // Phase 6: 2000 → 800（小店真实代运营 ¥2-4k/月）
    exposureBoost: 15,
    reputationBoost: 0,
    priceModifier: 1.0,
  ...
  },
  {
    ...
    unique: true,
    description:
      "开业活动，2周内曝光+40、口碑+5，售价7折（含物料、传单、优惠券）",
...
export const REPUTATION_ACTIVITIES: MarketingActivityConfig[] = [
  {
    ...
    type: "continuous",
    category: "reputation",
    baseCost: INGREDIENT_UPGRADE_WEEKLY_COST, // Phase 6: 800 → 500（升级食材成本应是 ¥0.5-1/杯）
    exposureBoost: 0,
    reputationBoost: 3,
    priceModifier: 1.0,
    dependencyCoefficient: 0,
...
export function getActivityConfig(
  activityId: string,
): MarketingActivityConfig | undefined {
 ...
 * 前4周满效果，之后逐步衰减到 30%
 */
export function calculateActivityEffectDecay(
  activeWeeks: number,
  dependencyCoeff: number,
): number {
 ...
 */
export function calculateStopPenalty(
  dependencyCoeff: number,
  activeWeeks: number,
): number {
 ...
 * 获取活动风险等级
 */
export function getActivityRiskLevel(
  dependencyCoeff: number,
): "low" | "medium" | "high" {
